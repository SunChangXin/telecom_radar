...
function rawTextBetween(block, tag) {
  const match = block.match(new RegExp(`<${tag}[^>]*>([\\s\\S]*?)<\\/${tag}>`, "i"));
  return stripHtml(match?.[1] || "");
}
...
function rssItemsFromXml(xml) {
  const blocks = xml.match(/<item\b[\s\S]*?<\/item>/gi) || xml.match(/<entry\b[\s\S]*?<\/entry>/gi) || [];
  return blocks.map((block) => {
    const linkTag = block.match(/<link\b[^>]*href=["']([^"']+)["'][^>]*\/?>/i);
    const link = rawTextBetween(block, "link") || linkTag?.[1] || "";
    const categories = [...block.matchAll(/<category[^>]*>([\s\S]*?)<\/category>/gi)]
      .map((match) => stripHtml(match[1]))
      .filter(Boolean);
    return {
      title: rawTextBetween(block, "title"),
      url: link,
      summary: rawTextBetween(block, "description") || rawTextBetween(block, "summary") || rawTextBetween(block, "content"),
      published_at: rawTextBetween(block, "pubDate") || rawTextBetween(block, "published") || rawTextBetween(block, "updated"),
      tags: categories
    };
  });
}
  ...
  for (const query of config.github.queries || []) {
    ...
    for (const repo of payload.items || []) {
      items.push({
        ...
        tags: [repo.language, ...(repo.topics || []), "open source", `★ ${repo.stargazers_count || 0}`].filter(Boolean)
      });
    }
  }
  return items;
...
function linkAllowed(link, feed) {
  const text = `${link.title} ${link.summary || ""} ${link.url} ${(link.tags || []).join(" ")}`.toLowerCase();
  const path = (() => {
...
async function fetchWebFeeds(config) {
  ...
  for (const feed of config.rss.feeds || []) {
    ...
    if (feed.type === "rss-feed") {
      const entries = rssItemsFromXml(body)
        .filter((entry) => entry.title && entry.url && linkAllowed(entry, feed))
        .slice(0, 20);
      for (const entry of entries) {
        items.push({
          source: feed.name,
          title: entry.title,
          url: entry.url,
          summary: entry.summary || `${feed.name} RSS 收录的相关论文动态。`,
      ...
      const links = linksFromHtml(body, feed.url)
